    ...
    case "$type" in
        code)
            fonts+=(
                ...
                ["CascadiaCode"]="https://github.com/ryanoasis/nerd-fonts/releases/download/v3.3.0/CascadiaCode.tar.xz"
            )
            ;;
        chinese)
            fonts+=(
...
main_install() {
    ...
    for font_info in "${font_list[@]}"; do
        ...
        if download_with_retry "$url" "$download_file"; then
            if extract_archive "$download_file" "$TEMP_DIR/$name"
            then
                for file in "$TEMP_DIR/$name"/*.{ttf,ttc,otf}; do
                    install_font "$file"
                done
            else
                log "ERROR" "无法解压 $name"
